...
class OgNumberInput {
    ...
    render() {
        return (__chunk_1.h(__chunk_1.Host, { class: { 'og-form-item__editor': true } }, __chunk_1.h("input", { type: "number", class: "og-input__input", value: this.value, step: this.step, min: this.min, max: this.max, disabled: this.disabled, onInput: (event) => this.handleChange(event), onFocus: (event) => this.focusGained.emit(event), onBlur: (event) => this.focusLost.emit(event), placeholder: this.placeholder }), __chunk_1.h("div", { class: "og-input__indicator" })));
    }
...
exports.og_number_input = OgNumberInput;
